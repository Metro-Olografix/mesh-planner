...
class Splat:
    def __init__(
        ...
        # Check the standard-resolution SPLAT! binaries exist and are executable.
        # HD binaries (splat-hd, srtm2sdf-hd) are checked lazily in coverage_prediction
        # when high_resolution=True is actually requested.
        ...
        if not os.path.isfile(self.srtm2sdf_binary) or not os.access(
            ...
            raise FileNotFoundError(
                f"'srtm2sdf' binary not found or not executable at '{self.srtm2sdf_binary}'"
            )
    ...
    def _create_splat_geotiff(
        ...
        try:
            ...
            # Read PPM content as RGB and reverse-map to colormap indices.
            # SPLAT! outputs RGB pixels coloured by signal level.  We must
            # map each pixel back to its colormap index (0–254) so that the
            # GeoTIFF stores a value that linearly represents dBm.
            # Previously this used img.convert("L") (BT.601 luminance) which
            # produced a non-linear, compressed mapping and ~5 dB errors.
            logger.debug("Reading PPM content.")
            with Image.open(io.BytesIO(ppm_bytes)) as img:
                img_rgb = np.array(img.convert("RGB"))  # (H, W, 3)

            logger.debug(f"PPM image dimensions: {img_rgb.shape[:2]}")

            # Build the 255-entry colormap LUT (same one used for the GDAL palette)
            ...
            for start in range(0, h * w, CHUNK):
                ...
                # Pixels far from any colourmap entry → nodata (255).
                # Threshold 900 ≈ (30 RGB-units)²; covers SPLAT!'s white background.
                best[min_dist > 900.0] = null_value
                img_idx[start:end] = best

            img_array = img_idx.reshape(h, w)
