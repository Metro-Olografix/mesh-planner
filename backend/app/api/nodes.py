...
def _serialize(node: Node) -> NodeOut:
    ...
    if node.coverage:
        cov_status = (
            "invalidated" if node.coverage.invalidated else node.coverage.status
        )
...
async def list_nodes(
    ...
    user_sub = user["sub"]
    visible = [n for n in nodes if n.status != "draft" or n.created_by == user_sub]
    logger.info(
...
async def create_node(
    ...
    await sse_manager.broadcast(
        "node_created", {"id": str(node.id), "name": node.name, "by": by}
    )
...
async def update_node(
    ...
    by = await get_display_name(user["_access_token"], user)
    logger.info(
        "Updating node %s ('%s') by %s — fields: %s",
        node_id,
        node.name,
        by,
    ...
    if update_data.keys() & _RF_FIELDS and node.coverage:
        logger.info(
            "RF-relevant fields changed for node %s — invalidating coverage cache",
            node_id,
        )
    ...
    db.add(
        NodeEvent(
            event_type="node_updated", node_id=str(node_id), node_name=node.name, by=by
        )
    )
    await db.commit()
    node = await _load_node(db, node_id)
    await sse_manager.broadcast(
        "node_updated", {"id": str(node.id), "name": node.name, "by": by}
    )
    return _serialize(node)
...
async def delete_node(
    ...
    db.add(
        NodeEvent(
            event_type="node_deleted", node_id=str(node_id), node_name=name, by=by
        )
    )
    ...
    await sse_manager.broadcast(
        "node_deleted", {"id": str(node_id), "name": name, "by": by}
    )
