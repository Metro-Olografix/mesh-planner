...
async def find_path(
    ...
    if not src_node:
        return PathResult(found=False, hops=[], bottleneck_snr_db=None,
                          total_relay_hops=0, message="Source node not found")
    if not dst_node:
        return PathResult(found=False, hops=[], bottleneck_snr_db=None,
                          total_relay_hops=0, message="Destination node not found")
    if src_node.id == dst_node.id:
        return PathResult(found=False, hops=[], bottleneck_snr_db=None,
                          total_relay_hops=0, message="Source and destination must be different nodes")
    ...
    for i, n in enumerate(nodes):
        hw = n.hardware
        gain = n.antenna_gain_dbi if n.antenna_gain_dbi is not None else hw.default_antenna_gain_dbi
    ...
    for k, idx in enumerate(path_indices):
        ...
        if k < len(path_indices) - 1:
            ...
            snr = round(matrix[idx][next_idx], 1) if matrix[idx][next_idx] is not None else None
        hops.append(HopInfo(node_id=gn.node_id, name=gn.name, lat=gn.lat, lon=gn.lon, snr_db=snr))

    relay_count = len(path_indices) - 2  # subtract source and destination
    if relay_count == 0:
        msg = (f"Direct link — {src_node.name} ↔ {dst_node.name} "
               f"(SNR: {bottleneck:.1f} dB, no relay needed)")
    else:
        msg = (f"Path via {relay_count} relay(s): "
